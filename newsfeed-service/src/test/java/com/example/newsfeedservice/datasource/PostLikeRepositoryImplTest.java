...
class PostLikeRepositoryImplTest {
    @Autowired
    PostLikeRepositoryImpl postLikeRepository;
    @Autowired
    ...
    @Test
    @DisplayName("포스트 좋아요 저장시 값이 올바르게 설정되어야 한다.")
    void save_post_like() {
        // given
        Long userId = 1L;
        PostEntity postEntity = savedPost.get(0);
        PostLikeEntity postLikeEntity = PostLikeEntity.create(userId, postEntity);

        // when
        postLikeRepository.save(postLikeEntity).getId();
        flushAndClearPersistence();
    ...
    @Test
    @DisplayName("저장된 포스트 좋아요는 조회가 가능해야 한다.")
    void find_post_like() {
        // given
        Long userId = 1L;
        PostEntity postEntity = savedPost.get(0);
        ...
        // then
        assertTrue(postLikeRepository.findByUserIdAndPostId(userId, postEntity.getId()).isPresent());
    }
    ...
    void find_post_like_list() {
        ...
        while (!postLikeIdList.isEmpty()) {
            List<PostLikeActivityDTO> likeActivityDTOS = postLikeRepository.findByIdList(postLikeIdList);
